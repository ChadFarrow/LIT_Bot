...
export class IRCClient {
  constructor(config) {
    ...
    this.joinedChannels = new Set(); // Track which channels we've successfully joined (stored lowercased)
    this.persistentConnection = process.env.IRC_PERSISTENT_CONNECTION !== 'false'; // Default to persistent
  }

  // IRC channel names are case-insensitive (RFC 2812), but the server may echo
  // JOINs in a different case than we requested (e.g. we ask for #HomegrownHits
  // and it replies #homegrownhits). Normalize so joined-state tracking matches
  // regardless of case — otherwise we falsely think a channel is unjoined and
  // time out trying to re-join it.
  markJoined(channel) {
  ...
  hasJoined(channel) {
    return this.joinedChannels.has(channel.toLowerCase());
  }
  ...
  async connect(targetChannels = null) {
    ...
    if (this.isConnected && this.client && this.persistentConnection) {
      ...
      const newChannels = channelsToJoin.filter(ch => !this.hasJoined(ch));
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        this.client = new irc.Client(ircServer, this.config.nickname, {
          ...
          // `node-icu-charset-detector` module on every message. ZeroNode is UTF-8,
          // so default decoding is correct and there is one fewer native module to
          // compile in the image. (BoostAfterBoost hit this as per-message log spam
          // when debug was on; debug is false here, so it was silent.)
          showErrors: true,
          debug: false
        });
  ...
  async joinChannels(channels) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const joinHandler = (channel, nick) => {
        if (nick === this.client.nick && channels.some(c => c.toLowerCase() === channel.toLowerCase())) {
          logger.info(`Successfully joined IRC channel: ${channel}`);
          this.markJoined(channel);
          channelsJoined++;
  ...
  setupEventHandlers(channelsToJoin, resolve, reject) {
    ...
    this.client.on('registered', () => {
      ...
      // NickServ: ghost any stale session and identify
      const nsPassword = process.env.IRC_NICKSERV_PASSWORD;
      if (nsPassword) {
        const desiredNick = this.config.nickname;
        if (this.client.nick !== desiredNick) {
          logger.info(`Nick is ${this.client.nick}, ghosting ${desiredNick} via NickServ`);
          this.client.say('NickServ', `GHOST ${desiredNick} ${nsPassword}`);
          // Reclaim nick after a short delay to let GHOST complete
          setTimeout(() => {
            this.client.send('NICK', desiredNick);
          }, 2000);
        }
        this.client.say('NickServ', `IDENTIFY ${nsPassword}`);
    ...
    this.client.on('error', (error) => {
      ...
      if (error.command === 'err_cannotsendtochan') {
        ...
        if (channel && this.hasJoined(channel)) {
  ...
  async postMessage(message, channels = null) {
    ...
    try {
      ...
      if (!this.isConnected || !this.client) {
        ...
        const missingChannels = targetChannels.filter(ch => !this.hasJoined(ch));
