...
export class IRCClient {
  ...
  async connect() {
    ...
    // Connect directly to IRC server (not through ZNC)
  ...
  setupEventHandlers() {
    ...
    this.client.on('ping', (server) => {
      // Only respond to pings after we're fully registered
      if (this.isConnected) {
        logger.debug('Received ping from server, sending pong');
        this.client.send('PONG', server);
      } else {
        logger.debug('Received ping before registration, ignoring');
      }
    });
  ...
  async postMessage(message, channels = null) {
    // If not connected, try to reconnect before posting
    if (!this.isConnectionActive()) {
      logger.warn('IRC not connected, attempting to reconnect before posting...');
      this.resetAndReconnect();
      
      // Wait up to 10 seconds for reconnection
      for (let i = 0; i < 10; i++) {
        await new Promise(resolve => setTimeout(resolve, 1000));
        if (this.isConnected && this.isConnectionActive()) {
          logger.info('IRC reconnection successful, proceeding with message');
          break;
        }
      }
      ...
      if (!this.isConnectionActive()) {
        logger.error('Failed to reconnect to IRC, cannot post message');
        return false;
      }
    }
  ...
  async ensureZncRunning() {
    try {
      ...
      if (!isHealthy) {
        ...
        const { stdout, stderr } = await execAsync('/home/server/bots/LIT_Bot/start-znc.sh');
        if (stderr) {
